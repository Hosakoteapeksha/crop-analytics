...
LOW_CONF_THRESHOLD = 50  # Below 50% = not a crop leaf image

def is_blurry(image: Image.Image) -> bool:
    """Detect if image is blurry using variance of grayscale pixel values."""
    img_gray = np.array(image.convert("L"), dtype=np.float32)
    # Calculate Laplacian variance — low value = blurry
    h, w = img_gray.shape
    laplacian = (
        img_gray[:-2, 1:-1] + img_gray[2:, 1:-1] +
        img_gray[1:-1, :-2] + img_gray[1:-1, 2:] -
        4 * img_gray[1:-1, 1:-1]
    )
    variance = float(np.var(laplacian))
    return variance < 80  # threshold for blur detection

def is_plant_image(image: Image.Image) -> bool:
    """Check if image has enough green content to be a plant/leaf image."""
    img = np.array(image.convert("RGB"), dtype=np.float32)
    r, g, b = img[:,:,0], img[:,:,1], img[:,:,2]
    # Count pixels where green channel dominates
    green_dominant = np.sum((g > r) & (g > b) & (g > 40))
...
def load_crop_model():
    """Load ONNX model - tiny and fast, no TensorFlow needed."""
    model_path = "crop_model.onnx"
    if not os.path.exists(model_path):
        return None, "Model file 'crop_model.onnx' not found."
    try:
...
def preprocess_image(image: Image.Image, target_size=(224, 224)):
    """Resize and preprocess image for MobileNetV2 (ONNX).
    Handles all image modes: RGB, RGBA, grayscale, palette etc.
    This fixes the bug where different users uploading different
    image formats always get Corn as output.
    """
    # Force convert to RGB first — critical fix
    img = image.convert("RGB")
    img = img.resize(target_size, Image.LANCZOS)
    arr = np.array(img, dtype=np.float32)

    # Safety check: ensure shape is (224, 224, 3)
    if arr.ndim == 2:
...
def infer(image: Image.Image, model) -> dict:
    ...
    if is_blurry(image):
        return {
            ...
            "confidence": 0,
            "top5": [],
            "error_msg": "❌ Image is too blurry! Please upload a clear, focused image of a crop leaf.",
        }

    # ── Check 2: Non-plant image detection ───────────────────────────────
    if not is_plant_image(image):
        return {
            "low_conf": True,
            "confidence": 0,
            "top5": [],
            "error_msg": "❌ This does not appear to be a plant or crop image! Please upload a clear image of a crop leaf.",
        }

    arr = preprocess_image(image)
    input_name = model.get_inputs()[0].name
    raw_preds  = model.run(None, {input_name: arr})
    preds      = raw_preds[0][0]  # shape: (38,)
    pred_idx   = int(np.argmax(preds))
    confidence = float(np.max(preds))
    conf_pct   = int(confidence * 100)
    top5_idx   = np.argsort(preds)[::-1][:5]
    top5       = [(PLANT_CLASSES[i] if i < len(PLANT_CLASSES) else ("Class " + str(i)),
                   round(float(preds[i]) * 100, 1)) for i in top5_idx]
    ...
    if conf_pct < LOW_CONF_THRESHOLD:
        return {
            ...
            "confidence": conf_pct,
            "top5": top5,
            "error_msg": f"❌ Could not identify this as a known crop! Model confidence is only {conf_pct}%. Please upload a clear image of a crop leaf such as Tomato, Potato, Corn, Apple etc.",
        }
...
with right_col:
    ...
    if r is None:
        ...
        err_msg = r.get("error_msg", "The model could not confidently identify this image.")
        st.markdown(f"""
        <div style="background:rgba(231,76,60,0.12);border:2px solid #e74c3c;border-radius:14px;
                    padding:1.5rem 1.8rem;margin-top:1rem;text-align:center;">
            <div style="font-size:3rem;margin-bottom:0.5rem;">⚠️</div>
            <div style="font-size:1.1rem;font-weight:700;color:#e74c3c;margin-bottom:0.5rem;">
                Invalid Image Detected
            </div>
            <div style="font-size:0.9rem;color:#c0392b;margin-bottom:1rem;">
                {err_msg}
            </div>
            <div style="background:rgba(231,76,60,0.08);border-radius:10px;padding:0.8rem;
                        font-size:0.82rem;color:#922b21;">
                <b>✅ Supported crops:</b> Tomato, Potato, Corn, Apple, Grape, Pepper,
                Strawberry, Peach, Orange, Soybean, Squash, Blueberry, Cherry, Raspberry<br><br>
                <b>📸 Image tips:</b> Upload a clear, well-lit, close-up photo of a single crop leaf.
                Avoid blurry, dark, or non-plant images.
            </div>
        </div>
        """, unsafe_allow_html=True)
